...
class ComfortReward(RewardFunction):
    ...
    def __init__(self, env_metadata: Mapping[str, Any], band: float = None, lower_exponent: float = None,
    ...
    def calculate(self, observations: List[Mapping[str, Union[int, float]]]) -> List[float]:
        ...
        for o in observations:
            ...
            if hvac_mode in [1, 2]:
                set_point = o['indoor_dry_bulb_temperature_cooling_set_point'] if hvac_mode == 1 else o[
    ...
    def calculate_ev_penalty(self, o: Mapping[str, Union[int, float, dict]], current_reward: float) -> float:
        ...
        for charger_id, data in ev_chargers.items():
            ...
            if last_charged_kwh > 0 and net_energy_before < 0:
                ...
                contributions["extra_self_production"] += -0.5 * self.weights[
                    "extra_self_production"] * penalty_multiplier
...
class VoltageReward(RewardFunction):

    def __init__(self, env_metadata, alpha: float = 200.0, **kwargs):
    ...
    def calculate(self, observations):
        ...
        if self.central_agent:
            v_all = np.array(observations[0]["bus_voltages"])
            r = np.mean(-(self.alpha * (v_all - 1.0)) ** 2 + 1.0)
            return [float(r)]

        else:
            rewards = []
            for o in observations:
                v_all = np.array(o["bus_voltages"])
                r = np.mean(-(self.alpha * (v_all - 1.0)) ** 2 + 1.0)

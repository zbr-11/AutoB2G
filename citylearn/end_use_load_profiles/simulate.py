...
class EndUseLoadProfilesEnergyPlusPartialLoadSimulator(EndUseLoadProfilesEnergyPlusSimulator):
    QUERIES_DIRECTORY = os.path.join(os.path.join(*Path(os.path.dirname(os.path.abspath(__file__))).parts[0:-1]),
                                     'misc', 'queries')

    def __init__(
            self, ideal_loads_simulator: EndUseLoadProfilesEnergyPlusSimulator, allow_multi_load_time_step: bool = None,
            multiplier_minimum: float = None,
            multiplier_maximum: float = None, multiplier_probability: float = None, output_variables: List[str] = None,
            output_meters: List[str] = None,
    ...
    def add_other_equipment(self, idf: IDF) -> IDF:
        ...
        # loads_filepath = os.path.join(self.output_directory, 'partial_load.csv')
        loads_filepath = os.path.join(self.output_directory, f'{self.simulation_id}.csv')
    ...
    def remove_ideal_loads_air_system(self, idf: IDF) -> IDF:
        ...
        obj_names = [
            ...
            'ZoneControl:Thermostat:OperativeTemperature', 'ZoneControl:Thermostat:TemperatureAndHumidity',

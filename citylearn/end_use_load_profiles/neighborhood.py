...
BuildingSimulators = Mapping[
    str,
    Union[EndUseLoadProfilesEnergyPlusSimulator, EndUseLoadProfilesEnergyPlusSimulator, List[
        EndUseLoadProfilesEnergyPlusPartialLoadSimulator]]
]
...
class NeighborhoodBuild:
    def __init__(
            self, schema_filepath: Path, citylearn_simulation_test_evaluation: pd.DataFrame,
            citylearn_simulation_lstm_prediction_data: Mapping[int, pd.DataFrame],
            citylearn_simulation_lstm_error_data: Mapping[int, pd.DataFrame],
            lstm_test_data: Mapping[int, pd.DataFrame], bldg_ids: List[int], sample_cluster_labels: List[int],
...
class Neighborhood:
    ...
    def build(
            self, idd_filepath: Union[Path, str], bldg_ids: List[int] = None, include_lstm_models: bool = None,
            test_lstm_models: bool = None,
            test_citylearn_simulation: bool = None, delete_energyplus_simulation_output: bool = None,
            sample_buildings_kwargs: Mapping[str, Any] = None,
            energyplus_simulation_kwargs: Mapping[str, Any] = None, train_lstm_kwargs: Mapping[str, Any] = None,
        ...
        test_citylearn_simulation = False if test_citylearn_simulation is None else test_citylearn_simulation
        ...
        if bldg_ids is None:
            ...
            meta = self.end_use_load_profiles.metadata.metadata.get()
        ...
        if include_lstm_models and test_lstm_models:
            ...
            for i, td in enumerate(lstm_training_data):
                try:
                    idx, data = self.test_lstm(i, td.copy(), schema_filepath)
                    lstm_test_data.append(data)
                except Exception as e:
                    lstm_test_data.append(None)
    ...
    def test_lstm(self, bldg_ix: int, training_data: pd.DataFrame, schema_filepath: Path) -> Tuple[int, pd.DataFrame]:
        ...
        # type cast
        columns = [c for c in observation_names + list(periodic_observations.keys()) if
                   not c.endswith('_sin') and not c.endswith('_cos')]
        ...
        for i, c in enumerate(observation_names):
            ...
            training_data[input_columns[-1]] = training_data[c] * Normalize(normalization_minimum[i],
                                                                            normalization_maximum[i])
        ...
        for _, reference_data in training_data.groupby('reference'):
            ...
            reference_data = reference_data[
                ['timestep', 'reference', 'reference_name', actual_column, predicted_column]].copy()
            data_list.append(reference_data)

        return bldg_ix, pd.concat(data_list, ignore_index=True)

    def test_citylearn_simulation(
            self, schema: Path, model: Agent = None, env_kwargs: Mapping[str, Any] = None,
        ...
        if report_lstm_performance:
            ...
            for b in env.buildings:
                ...
                rmse_data = lstm_prediction_data[-1].groupby(['month'])[
                    ['indoor_dry_bulb_temperature', 'indoor_dry_bulb_temperature_predicted']].apply(
                    lambda x: mean_squared_error(x['indoor_dry_bulb_temperature'],
                                                 x['indoor_dry_bulb_temperature_predicted']
                                                 )).reset_index(name='rmse')
                rmse_data['rmse'] = rmse_data['rmse'] ** 0.5
                mape_data = lstm_prediction_data[-1].groupby(['month'])[
                    ['indoor_dry_bulb_temperature', 'indoor_dry_bulb_temperature_predicted']].apply(
                    lambda x: mean_absolute_percentage_error(x['indoor_dry_bulb_temperature'],
                                                             x['indoor_dry_bulb_temperature_predicted']
    ...
    def set_schema(
            ...
            carbon_intensity: List[Union[Path, pd.DataFrame, str]] = None,
            template: Mapping[str, Union[dict, float, int, str]] = None,
            metadata: pd.DataFrame = None, dataset_name: str = None, schema_directory: Union[Path, str] = None,
            weather_kwargs: dict = None,
        ...
        assert carbon_intensity is None or not isinstance(carbon_intensity, list) or len(carbon_intensity) == len(
            bldg_ids), \
        ...
        _ = shutil.copy(reference_simulator.epw_filepath,
                        os.path.join(schema_directory, weather_filename.replace('.csv', '.epw')))
        ...
        for i, (bldg_id, building_simulators, lstm_model) in enumerate(zip(bldg_ids, simulators, lstm_models)):
            ...
            print("📁 simulator.QUERIES_DIRECTORY =", simulator.QUERIES_DIRECTORY)

            # set building data
            building_data_query_filepath = os.path.join(simulator.QUERIES_DIRECTORY,
                                                        'select_citylearn_energy_simulation.sql')
            building_data = simulator.get_output_database().query_table_from_file(building_data_query_filepath)
            building_data_ideal = ideal_simulator.get_output_database().query_table_from_file(
                building_data_query_filepath)
            building_data['indoor_dry_bulb_temperature_cooling_set_point'] = building_data_ideal[
                'indoor_dry_bulb_temperature_cooling_set_point'].tolist()
            building_data['indoor_dry_bulb_temperature_heating_set_point'] = building_data_ideal[
                'indoor_dry_bulb_temperature_heating_set_point'].tolist()
            ...
            if no_lstm_model:
                ...
                building['dynamics']['attributes']['input_observation_names'] = lstm_model['attributes'][
                    'input_observation_names']
                building['dynamics']['attributes']['input_normalization_minimum'] = lstm_model['attributes'][
                    'input_normalization_minimum']
                building['dynamics']['attributes']['input_normalization_maximum'] = lstm_model['attributes'][
    ...
    def __get_building_pricing_and_carbon_intensity(
        ...
        data = dict(
            pricing=dict(
                default_filename=building_template['pricing'] if building_template[
                                                                     'pricing'] is not None else 'pricing.csv',
                data=pricing,
                schema_filename=None
            ),
            carbon_intensity=dict(
                default_filename=building_template['carbon_intensity'] if building_template[
                                                                              'carbon_intensity'] is not None else 'carbon_intensity.csv',
    ...
    def get_weather_data(self, simulator: EndUseLoadProfilesEnergyPlusPartialLoadSimulator,
                         shifts: Tuple[int, int, int] = None,
                         accuracy: Mapping[str, Tuple[float, float, float]] = None) -> pd.DataFrame:
        ...
        accuracy = {c: (0.3, 0.65, 1.35) if c == 'outdoor_dry_bulb_temperature' else (0.025, 0.05, 0.1) for c in
                    columns} \
    ...
    def train_lstm(self, data: List[pd.DataFrame], config: Mapping[str, Any] = None, seed: int = None) -> List[
        Mapping[str, Any]]:
    ...
    def get_lstm_training_data(self, simulators: BuildingsSimulators) -> List[pd.DataFrame]:
        ...
        for building_simulators in simulators:
            ...
            for partial_simulator in building_simulators['partial']:
                ...
                query_filepath = os.path.join(EndUseLoadProfilesEnergyPlusPartialLoadSimulator.QUERIES_DIRECTORY,
                                              'select_lstm_training_data.sql')
    ...
    def simulate_energy_plus(
            self, bldg_ids: List[int], idd_filepath: Union[Path, str], simulation_ids: List[str] = None,
            models: List[Union[Path, str]] = None,
            schedules: List[Union[Path, pd.DataFrame]] = None,
            thermostat_setpoints: List[Union[Path, pd.DataFrame]] = None,
            fixed_ideal_loads_idf: List[Union[Path, str]] = None, osm: bool = None,
            partial_loads_simulations: int = None, simulate_mechanical: bool = None,
        ...
        if (kwargs.get('default_output_variables', None) is None or not kwargs['default_output_variables']) and kwargs.get('output_variables', None) is None:
            kwargs['default_output_variables'] = True

        if (kwargs.get('default_output_meters', None) is None or not kwargs['default_output_meters']) and kwargs.get('output_meters', None) is None:
            kwargs['default_output_meters'] = True

        for i, bldg_id in enumerate(bldg_ids):
            bldg_id_ix = bldg_ids[:i].count(bldg_id)
            simulation_id = f'build-{bldg_id}-{bldg_id_ix}' if simulation_ids is None else simulation_ids[i]
            ...
            for partial_i in range(partial_loads_simulations + 2):
                ...
                if partial_i == 0:
                    ...
                    reference_name = 'under_over_cool_heat'

                _simulation_id = f'{simulation_id}-partial-{reference_name}-{partial_i}'
                _output_directory = os.path.join(output_directory, _simulation_id)
                ...
                partial_loads_simulators.append(
                    EndUseLoadProfilesEnergyPlusPartialLoadSimulator(
                        ...
                        simulation_id=_simulation_id,
                        output_directory=_output_directory,
                        random_seed=int(bldg_id) + partial_i,
                        **partial_loads_kwargs
                    )
                )

            # EndUseLoadProfilesEnergyPlusPartialLoadSimulator.multi_simulate(partial_loads_simulators, max_workers=1)
            for ps in partial_loads_simulators:
                try:
                    ps.simulate()
                    print(f"   ✅ {ps.simulation_id} completed")
                except Exception as e:
                    print(f"   ❌ {ps.simulation_id} failed: {e}")
    ...
    def simulate_energy_plus1(
        ...
        assert thermostat_setpoints is None or len(thermostat_setpoints) == len(
            bldg_ids), 'There must be as many thermostat_setpoints as bldg_ids.'
        assert fixed_ideal_loads_idf is None or len(fixed_ideal_loads_idf) == len(
            bldg_ids), 'There must be as many fixed_ideal_loads_idf as bldg_ids.'
        assert simulation_ids is None or len(simulation_ids) == len(
            bldg_ids), 'There must be as many simulation_ids as bldg_ids.'
        os.makedirs(self.energyplus_output_directory, exist_ok=True)
        partial_loads_simulations = 4 if partial_loads_simulations is None else partial_loads_simulations
        simulate_mechanical = True if simulate_mechanical is None else simulate_mechanical
        delete_existing_simulation = True if delete_existing_simulation is None else delete_existing_simulation
        ...
        kwargs = dict(
            ...
            osm=osm,
            **kwargs,
            number_of_time_steps_per_hour=1 if kwargs.get('number_of_time_steps_per_hour', None) is None else kwargs[
                'number_of_time_steps_per_hour']
        )
        ...
        for i, bldg_id in enumerate(bldg_ids):
            bldg_id_ix = bldg_ids[:i].count(bldg_id)
            simulation_id = f'{self.end_use_load_profiles.version}-{bldg_id}-{bldg_id_ix}' if simulation_ids is None else \
            simulation_ids[i]
            simulation_id = f'build-{bldg_id}-{bldg_id_ix}' if simulation_ids is None else \
                simulation_ids[i]
            output_directory = os.path.join(self.energyplus_output_directory, simulation_id)
            ...
            _kwargs = dict(
                **kwargs,
                ...
                model=models[i] if models is not None else models,
                schedules=schedules[i] if schedules is not None else schedules,
                thermostat_setpoint=thermostat_setpoints[
                    i] if thermostat_setpoints is not None else thermostat_setpoints,
            )
            ...
            EndUseLoadProfilesEnergyPlusPartialLoadSimulator.multi_simulate(partial_loads_simulators,
                                                                            max_workers=1)
    ...
    def sample_buildings(
            self, filters: Mapping[str, List[Any]] = None, sample_method: SampleMethod = None, sample_count: int = None,
            duplicate_to_count: bool = None,
        ...
        if sample_method == SampleMethod.RANDOM:
            ...
            mc = MetadataClustering(self.end_use_load_profiles, metadata.index.tolist(), random_seed=self.random_seed,
                                    **kwargs)
            ...
            metadata = metadata.merge(labels[labels['clusters'] == optimal_clusters].set_index('bldg_id'),
                                      left_index=True, right_index=True)

        metadata['label_count'] = metadata.groupby('label')['label'].transform('count')
        metadata = metadata.sample(sample_count, weights='label_count', replace=duplicate_to_count,

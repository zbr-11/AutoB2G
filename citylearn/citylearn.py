...
from citylearn.data import CarbonIntensity, DataSet, ChargerSimulation, EnergySimulation, \
    LogisticRegressionOccupantParameters, Pricing, WashingMachineSimulation, Weather
...
class CityLearnEnv(Environment, Env):
    ...
    **kwargs : dict
    ...
    def __init__(self,
                 ...
                 simulation_start_time_step: int = None, simulation_end_time_step: int = None,
                 episode_time_steps: Union[int, List[Tuple[int, int]]] = None, rolling_episode_split: bool = None,
                 random_episode_split: bool = None, seconds_per_time_step: float = None,
                 reward_function: Union[RewardFunction, str] = None, reward_function_kwargs: Mapping[str, Any] = None,
                 central_agent: bool = None, shared_observations: List[str] = None,
                 ...
                 inactive_actions: Union[List[str], List[List[str]]] = None, simulate_power_outage: bool = None,
                 solar_generation: bool = None, random_seed: int = None, time_step_ratio: int = None,
                 start_date: Union[str, datetime.date] = None, render_session_name: str = None,
                 render_mode: str = 'none', **kwargs: Any
                 ):
        ...
        self._render_start_date = self._parse_render_start_date(
            start_date if start_date is not None else schema_start_date)
        ...
        if self.render_session_name is not None:
            ...
            if self.render_session_name == '':
                ...
                raise ValueError(
                    'render_session_name must be a relative path. Use render_directory to choose an absolute location.')
        ...
        super().__init__(seconds_per_time_step=seconds_per_time_step, random_seed=self.random_seed,
                         episode_tracker=episode_tracker, time_step_ratio=self.time_step_ratio)
        ...
        self._pp_net = pn.case33bw()

        # -------- 1) 先删掉电网自带的全部负荷 --------
        self._pp_net.load.drop(self._pp_net.load.index, inplace=True)
        ...
        self.bus_voltages = [1.0] * len(self._pp_net.bus)
        self.building_number= node_building_number
    ...
    def observation_space(self) -> List[spaces.Box]:
        ...
        if self.central_agent:
            ...
            #     low_limit.append(0.75)
            #     high_limit.append(1.25)
            low_limit += [0.90] * len(self._pp_net.bus)
            high_limit += [1.10] * len(self._pp_net.bus)
            ...
            for b in self.buildings:
                low = list(b.observation_space.low)
                high = list(b.observation_space.high)

                # low += [0.90] * len(self._pp_net.bus)
                # high += [1.10] * len(self._pp_net.bus)

                observation_space.append(spaces.Box(low=np.array(low, dtype=np.float32),high=np.array(high, dtype=np.float32),dtype=np.float32))
            ...
            for i, b in enumerate(self.buildings):
                for k, v in b.observations(normalize=False, periodic_normalization=False, check_limits=True).items():
                    ...
                    if k in self.shared_observations and k not in shared_observations:
                        ...
                        pass
                # observations.append(getattr(b, "vm_pu", 1.0))
            observations.extend(self.bus_voltages)

            observations = [observations]

        else:
            # observations = [
            #     list(b.observations(normalize=False, periodic_normalization=False, check_limits=True).values()) for b in
            #     self.buildings]
            observations = []

            for b in self.buildings:
                obs_b = list(b.observations(normalize=False,periodic_normalization=False,check_limits=True).values())
                # obs_b = obs_b + list(self.bus_voltages)
    ...
    def observation_names(self) -> List[List[str]]:
        ...
        if self.central_agent:
            ...
            for i, b in enumerate(self.buildings):
                for k, _ in b.observations(normalize=False, periodic_normalization=False).items():
                    if i == 0 or k not in self.shared_observations or k not in observation_names:
                        ...
                        pass
                # observation_names.append(f"vm_pu_b{i}")
            observation_names += [f"bus_vm_pu_{i}" for i in range(len(self._pp_net.bus))]
            ...
            for b in self.buildings:
                ...
                # names += [f"bus_vm_pu_{i}" for i in range(len(self._pp_net.bus))]
    ...
    def energy_to_non_shiftable_load(self) -> np.ndarray:
        ...
        return pd.DataFrame([b.energy_to_non_shiftable_load for b in self.buildings]).sum(axis=0,
                                                                                          min_count=1).to_numpy()
    ...
    def power_outage(self) -> np.ndarray:
        ...
        return pd.DataFrame([b.power_outage_signal for b in self.buildings]).sum(axis=0, min_count=1).to_numpy()[
               :self.time_step + 1]
    ...
    def _run_pandapower(self):
        net = self._pp_net
        if self.building_number is None:
            self.building_number = len(self.buildings)

        for class_id, b in enumerate(self.buildings):
            p_single = float(b.net_electricity_consumption[self.time_step]) / 1000 * (self.building_number/len(self.buildings))

            for bus in range(1, len(net.bus)):
                ...
                if existing:
                    ...
                    idx = pp.create_load(net, bus=bus, p_mw=0.0, q_mvar=0.0, name=load_name)

                net.load.at[idx, "p_mw"] = p_single

        pp.runpp(net)

        self.bus_voltages = net.res_bus.vm_pu.values.copy()
    ...
    def step(self, actions: List[List[float]]) -> Tuple[List[List[float]], List[float], bool, bool, dict]:
        ...
        reward_observations = [b.observations(include_all=True, normalize=False, periodic_normalization=False) for b in
                               self.buildings]
        # reward_observations[0]["bus_voltages"] = list(self.bus_voltages)
    ...
    def _parse_actions(self, actions: List[List[float]]) -> List[Mapping[str, float]]:
        ...
        for i, building in enumerate(self.buildings):
            ...
            if electric_vehicle_actions:
                action_dict[
                    'electric_vehicle_storage_actions'] = electric_vehicle_actions  # aqui podes criar dicionario
    ...
    def evaluate(self, control_condition: EvaluationCondition = None, baseline_condition: EvaluationCondition = None,
                 comfort_band: float = None) -> pd.DataFrame:
        ...
        get_net_electricity_consumption_emission = lambda x, c: getattr(x,
                                                                        f'net_electricity_consumption_emission{c.value}')
        ...
        for b in self.buildings:
            ...
            ce_b = CostFunction.carbon_emissions(get_net_electricity_consumption_emission(b, baseline_condition))[
                -1] if sum(b.carbon_intensity.carbon_intensity) != 0 else 0
            cost_c = CostFunction.cost(get_net_electricity_consumption_cost(b, control_condition))[-1]
            cost_b = CostFunction.cost(get_net_electricity_consumption_cost(b, baseline_condition))[-1] if sum(
                b.pricing.electricity_pricing) != 0 else 0
            ...
            building_level_ = pd.DataFrame([{
                ...
                'value':
                    CostFunction.one_minus_thermal_resilience(power_outage=b.power_outage_signal, **discomfort_kwargs)[
                        -1],
            }, {
                'cost_function': 'power_outage_normalized_unserved_energy_total',
                'value': CostFunction.normalized_unserved_energy(expected_energy, served_energy,
                                                                 power_outage=b.power_outage_signal)[-1]
            }, {
        ...
        dlf24_c = \
        CostFunction.one_minus_load_factor(get_net_electricity_consumption(self, control_condition), window=24)[-1]
        dlf24_b = \
        CostFunction.one_minus_load_factor(get_net_electricity_consumption(self, baseline_condition), window=24)[-1]
        dlf730_c = \
        CostFunction.one_minus_load_factor(get_net_electricity_consumption(self, control_condition), window=730)[-1]
        dlf730_b = \
        ...
        peak_all_c = \
        CostFunction.peak(get_net_electricity_consumption(self, control_condition), window=self.time_steps)[-1]
        peak_all_b = \
    ...
    def associate_chargers_to_electric_vehicles(self):
        ...
        def _resolve_arrival_soc(simulation: ChargerSimulation, step: int, prev_state: float, prev_id: Union[str, None],
                                 ev_identifier: str) -> Union[float, None]:
            ...
            if candidate_index is not None and 0 <= candidate_index < len(
        ...
        for ev in self.electric_vehicles:
            ...
            for building in self.buildings:
                for charger in building.electric_vehicle_chargers or []:
                    ...
                    curr_state = sim.electric_vehicle_charger_state[t] if t < len(
                        sim.electric_vehicle_charger_state) else np.nan
                    next_state = sim.electric_vehicle_charger_state[t + 1] if t + 1 < len(
                        sim.electric_vehicle_charger_state) else np.nan
    ...
    def _ensure_render_output_dir(self, *, ensure_exists: bool = True):
        ...
        base_render_path = Path(
            getattr(self, 'render_output_root', Path(__file__).resolve().parents[1] / 'render_logs')).expanduser()
    ...
    def load_agent(self, agent: Union[str, 'citylearn.agents.base.Agent'] = None, **kwargs) -> Union[
        Any, 'citylearn.agents.base.Agent']:
    ...
    def _load(self, schema: Mapping[str, Any], **kwargs) -> Tuple[
        Union[Path, str], List[Building], List[ElectricVehicle], Union[
            int, List[Tuple[int, int]]], bool, bool, float, RewardFunction, bool, List[str], EpisodeTracker]:
        ...
        schema['chargers_observations_helper'] = {key: value for key, value in schema["observations"].items() if
                                                  "electric_vehicle_" in key}
        schema['chargers_actions_helper'] = {key: value for key, value in schema["actions"].items() if
                                             "electric_vehicle_" in key}
        schema['chargers_shared_observations_helper'] = {key: value for key, value in schema["observations"].items()
                                                         if "electric_vehicle_" in key and value.get(
                "shared_in_central_agent", True)}

        schema['washing_machine_observations_helper'] = {key: value for key, value in schema["observations"].items() if
                                                         "washing_machine_" in key}
        schema['washing_machine_actions_helper'] = {key: value for key, value in schema["actions"].items() if
                                                    "washing_machine" in key}

        schema['observations'] = {
            key: value
            for key, value in schema["observations"].items()
            if
            key not in set(schema['chargers_observations_helper']) | set(schema['washing_machine_observations_helper'])
        }
        schema['actions'] = {
            key: value
        ...
        schema['episode_time_steps'] = kwargs['episode_time_steps'] if kwargs.get(
            'episode_time_steps') is not None else schema.get('episode_time_steps', None)
        schema['rolling_episode_split'] = kwargs['rolling_episode_split'] if kwargs.get(
            'rolling_episode_split') is not None else schema.get('rolling_episode_split', None)
        schema['random_episode_split'] = kwargs['random_episode_split'] if kwargs.get(
            'random_episode_split') is not None else schema.get('random_episode_split', None)
        schema['seconds_per_time_step'] = kwargs['seconds_per_time_step'] if kwargs.get(
            'seconds_per_time_step') is not None else schema['seconds_per_time_step']
        ...
        for i, building_name in enumerate(buildings_to_include):
            buildings.append(
                self._load_building(i, building_name, schema, episode_tracker, pv_sizing_data, battery_sizing_data,
                                    **kwargs))
        ...
        for electric_vehicle_name, electric_vehicle_schema in electric_vehicle_schemas.items():
            if electric_vehicle_schema['include']:
                ...
                electric_vehicles.append(
                    self._load_electric_vehicle(electric_vehicle_name, schema, electric_vehicle_schema, episode_tracker,
                                                time_step_ratio))
        ...
        return (
            schema['root_directory'], buildings, electric_vehicles, schema['episode_time_steps'],
    ...
    def _load_building(self, index: int, building_name: str, schema: dict, episode_tracker: EpisodeTracker,
        ...
        energy_simulation = EnergySimulation(**energy_simulation.to_dict('list'),
                                             seconds_per_time_step=seconds_per_time_step, noise_std=noise_std)
        ...
        if building_schema.get('occupant', None) is not None:
            ...
            for k in ['increase', 'decrease']:
                attributes[f'setpoint_{k}_model_filepath'] = os.path.join(schema['root_directory'],
                                                                          attributes[f'setpoint_{k}_model_filename'])
        ...
        if building_schema.get("chargers", None) is not None:
            for charger_name, charger_config in building_schema["chargers"].items():
                ...
                charger_object = charger_class(charger_simulation=charger_simulation, charger_id=charger_name,
                                               **charger_attributes,
                                               seconds_per_time_step=schema['seconds_per_time_step'],
        ...
        for washing_machine_name, washing_machine_schema in washing_machine_schemas.items():
            washing_machines_list.append(
                self._load_washing_machine(washing_machine_name, schema, washing_machine_schema, episode_tracker))

        observation_metadata, action_metadata = self.process_metadata(schema, building_schema, chargers_list,
                                                                      washing_machines_list, index, energy_simulation,
                                                                      **kwargs)
    ...
    def process_metadata(self, schema, building_schema, chargers_list, washing_machines_list, index,
                         energy_simulation: EnergySimulation, **kwargs):
        ...
        chargers_observations_metadata_helper = {k: v['active'] for k, v in
                                                 schema['chargers_observations_helper'].items()}
        washing_machine_observations_metadata_helper = {k: v['active'] for k, v in
                                                        schema['washing_machine_observations_helper'].items()}
        ...
        washing_machine_actions_metadata_helper = {k: v['active'] for k, v in
                                                   schema['washing_machine_actions_helper'].items()}
        ...
        if kwargs.get('active_actions') is not None:
            ...
            chargers_actions_metadata_helper = {k: True if k in active_actions else False for k in
                                                chargers_actions_metadata_helper}
            washing_machine_actions_metadata_helper = {k: True if k in active_actions else False for k in
        ...
        chargers_actions_metadata_helper = {k: False if k in inactive_actions else v for k, v in
                                            chargers_actions_metadata_helper.items()}
        washing_machine_actions_metadata_helper = {k: False if k in inactive_actions else v for k, v in
                                                   washing_machine_actions_metadata_helper.items()}
        ...
        if len(chargers_list) > 0:
            for charger in chargers_list:  # If present, iterate each charger
                ...
                if chargers_observations_metadata_helper.get("connected_electric_vehicle_at_charger_departure_time",
                                                             False):
                ...
                if chargers_observations_metadata_helper.get(
                        "connected_electric_vehicle_at_charger_required_soc_departure", False):
                    observation_metadata[
                        f'connected_electric_vehicle_at_charger_{charger_id}_required_soc_departure'] = True
                ...
                if chargers_observations_metadata_helper.get("connected_electric_vehicle_at_charger_battery_capacity",
                                                             False):
                ...
                if chargers_observations_metadata_helper.get(
                        ...
                        False):
                    observation_metadata[
                        f'incoming_electric_vehicle_at_charger_{charger_id}_estimated_arrival_time'] = True
    ...
    def _load_electric_vehicle(self, electric_vehicle_name: str, schema: dict, electric_vehicle_schema: dict,
                               episode_tracker: EpisodeTracker, time_step_ratio) -> ElectricVehicle:
        ...
        electric_vehicle_constructor = getattr(importlib.import_module(electric_vehicle_type_module),
                                               electric_vehicle_type_name)
